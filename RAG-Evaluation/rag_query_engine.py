...
class RAGQueryEngine:
    ...
    def _initialize_bi_encoder(self) -> None:
        """Initialize bi-encoder model with better similarity model."""
        try:
            # Use a more powerful model for better semantic similarity
    ...
    def rerank_biencoder(self, chunks: List[Dict], query: str, k: int = 1) -> List[Tuple[str, float]]:
        """
        Rerank chunks using bi-encoder cosine similarity and return top-k with scores.
        ...
        Returns:
            List of tuples (chunk_text, cosine_score) sorted by score
        """
        try:
            chunk_texts = [chunk["text"] for chunk in chunks]
            ...
            chunk_embeddings = self.bi_encoder.encode(
                chunk_texts, 
                convert_to_tensor=True,
                normalize_embeddings=True
            )
